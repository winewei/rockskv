#!/bin/bash
# Start RocksKV Storage Service (Node 4)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
CONFIG_FILE="${CONFIG_FILE:-$PROJECT_DIR/config/local/storage-4.yaml}"
...
DATA_DIR="./data/storage4"
mkdir -p "$DATA_DIR/wal"
mkdir -p "./data/sst4"

echo "Starting RocksKV Storage Service (Node 4)..."
echo "  Binary: $BINARY"
