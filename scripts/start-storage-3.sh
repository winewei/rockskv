#!/bin/bash
# Start RocksKV Storage Service (Node 3)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
CONFIG_FILE="${CONFIG_FILE:-$PROJECT_DIR/config/local/storage-3.yaml}"
...
DATA_DIR="./data/storage3"
mkdir -p "$DATA_DIR/wal"
mkdir -p "./data/sst3"

echo "Starting RocksKV Storage Service (Node 3)..."
echo "  Binary: $BINARY"
